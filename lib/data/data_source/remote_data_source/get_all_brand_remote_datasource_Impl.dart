  ...
  Future<Either<Faliures, CategoryAndBrandResponseDm>> getAllBrands() async {
    try {
      final Response response =
          await apiManager.getData(endPoint: ApiEndPoint.allBrands);
      CategoryAndBrandResponseDm categoryAndBrandResponseDm =
          CategoryAndBrandResponseDm.fromJson(response.data);
      if (response.statusCode! >= 200 && response.statusCode! < 300) {
        return Right(categoryAndBrandResponseDm);
      } else {
        return Left(
            ServerError(errMessage: categoryAndBrandResponseDm.message!));
      }
    } catch (e) {
